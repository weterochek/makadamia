...
app.get('/product/:name', async (req, res) => {
    const productName = req.params.name;  // Получаем название продукта
    try {
        const product = await Products.findOne({ name: productName });  // Ищем по имени
        if (!product) {
    ...
    try {
        ...
        const newOrder = new Order({
            ...
            items: items.map(item => ({
                productId: item.productId, // Получаем ID товара
                name: item.name,           // Получаем название товара
                price: item.price,         // Получаем цену товара
